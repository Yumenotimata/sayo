...
typedef kk_datatype_ptr_t kk_interpret__value;
...
static inline bool kk_interpret_is_int(kk_interpret__value value, kk_context_t* _ctx) { /* (value : value) -> bool */ 
  if (kk_interpret__is_Int(value, _ctx)) {
    struct kk_interpret_Int* _con_x186 = kk_interpret__as_Int(value, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_fnVal(kk_interpret__value value, kk_context_t* _ctx) { /* (value : value) -> bool */ 
  if (kk_interpret__is_FnVal(value, _ctx)) {
    struct kk_interpret_FnVal* _con_x187 = kk_interpret__as_FnVal(value, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_cont(kk_interpret__value value, kk_context_t* _ctx) { /* (value : value) -> bool */ 
  if (kk_interpret__is_Cont(value, _ctx)) {
    struct kk_interpret_Cont* _con_x188 = kk_interpret__as_Cont(value, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_lit(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_Lit(expr, _ctx)) {
    struct kk_interpret_Lit* _con_x189 = kk_interpret__as_Lit(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_var(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_Var(expr, _ctx)) {
    struct kk_interpret_Var* _con_x190 = kk_interpret__as_Var(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_let(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_Let(expr, _ctx)) {
    struct kk_interpret_Let* _con_x191 = kk_interpret__as_Let(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_fn(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_Fn(expr, _ctx)) {
    struct kk_interpret_Fn* _con_x192 = kk_interpret__as_Fn(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_app(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_App(expr, _ctx)) {
    struct kk_interpret_App* _con_x193 = kk_interpret__as_App(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_reset(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_Reset(expr, _ctx)) {
    struct kk_interpret_Reset* _con_x194 = kk_interpret__as_Reset(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline bool kk_interpret_is_shift(kk_interpret__expr expr, kk_context_t* _ctx) { /* (expr : expr) -> bool */ 
  if (kk_interpret__is_Shift(expr, _ctx)) {
    struct kk_interpret_Shift* _con_x195 = kk_interpret__as_Shift(expr, _ctx);
    return true;
  }
  {
    return false;
...
static inline kk_std_core_types__list kk_interpret_extend(kk_std_core_types__list prev, kk_std_core_types__tuple2 kkloc_new, kk_context_t* _ctx) { /* (prev : env, new : (string, value)) -> env */ 
  kk_std_core_types__list ys_10003 = kk_std_core_types__new_Cons(kk_reuse_null, 0, kk_std_core_types__tuple2_box(kkloc_new, _ctx), kk_std_core_types__new_Nil(_ctx), _ctx); /*list<(string, interpret/value)>*/;
  if (kk_std_core_types__is_Nil(prev, _ctx)) {
    return ys_10003;
  }
  {
    return kk_std_core_list__unroll_append_10004(prev, ys_10003, _ctx);
  }
}
...
struct kk_interpret_eval_fun282__t {
  struct kk_function_s _base;
};
extern kk_interpret__value kk_interpret_eval_fun282(kk_function_t _fself, kk_interpret__value x, kk_context_t* _ctx);
static inline kk_function_t kk_interpret_new_eval_fun282(kk_context_t* _ctx) {
  kk_define_static_function(_fself, kk_interpret_eval_fun282, _ctx)
  return kk_function_dup(_fself,kk_context());
}


static inline kk_interpret__value kk_interpret_eval(kk_interpret__expr e, kk_context_t* _ctx) { /* (e : expr) -> pure value */ 
  return kk_interpret_eval_sq_(e, kk_std_core_types__new_Nil(_ctx), kk_interpret_new_eval_fun282(_ctx), _ctx);
}
 
// monadic lift

static inline kk_std_core_exn__error kk_interpret__mlift_run_10057(kk_interpret__value _y_x10043, kk_context_t* _ctx) { /* (value) -> pure error<value> */ 
  return kk_std_core_exn__new_Ok(kk_interpret__value_box(_y_x10043, _ctx), _ctx);
}
